...
function convertX(scope, db, meta) {
    ...
    for (var timeslot = 0; timeslot < slotsBetween; timeslot++) {
        ...
        for (var user in ret.monthsPuffer[timeslot]) {
            var userInSlot = openedSlots.find(function(e) {
                return e ? (e.user.name == user) : false
            });
        ...
        if (timeslot > 0) {
            ...
            for (var user in endingUsers) {
                ...
                var endingUserSlot = openedSlots.find(function(e) {
                    return e ? (e.user.name == user) : false
                });
    ...
    // order by cnt
    var orderedUsers = Object.keys(userset).map(function(name) {
        return userset[name]
    });
